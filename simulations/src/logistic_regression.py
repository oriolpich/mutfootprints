"""
Given a treatment, produce a collection of signatures
that are statistically associated to the treatment
after adjusting by the tumor type.
"""
...
def create_matrix_treatments(sig, n_treated, replicate, deconstruction_folder, shuffle=None):

    if shuffle is None:
        fol_name = f'{sig}.{n_treated}.{replicate}'
        exposure = pd.read_csv(os.path.join(deconstruction_folder,
                                            f'{fol_name}/{fol_name}.exposures.tsv'),
                               sep='\t')
    else:
        fol_name = f'{sig}.{n_treated}.{replicate}'
        exposure = pd.read_csv(os.path.join(deconstruction_folder,
                                            f'{fol_name}/shuffle.{shuffle}.{fol_name}.exposures.tsv'),
                               sep='\t')
...
def ensemble_regression(sig, n_treated, replicate, output_folder, deconstruction_folder, shuffle=None):

    # return the matrix, the signatures list and the tumor type list

    raw_regression_table = create_matrix_treatments(sig, n_treated, replicate, deconstruction_folder, shuffle=shuffle)
    ...
    if shuffle is None:
        with open(os.path.join(output_folder,
                               f'regression_lor.{sig}.{n_treated}.{replicate}.json'), 'wt') as f:
            json.dump(merge_res, f)
    else:
        with open(os.path.join(output_folder,
                               f'shuffle.{shuffle}.regression_lor.{sig}.{n_treated}.{replicate}.json'), 'wt') as f:
            json.dump(merge_res, f)
    ...
    if shuffle is None:
        res.to_csv(os.path.join(output_folder,
...
@cli.command()
@click.option('--sig', type=str)
@click.option('--deconstruction_folder', type=click.Path())
def treatment_shuffle(sig, deconstruction_folder):

    np.random.seed(42)
    for fn in tqdm(glob.glob(os.path.join(deconstruction_folder, f'{sig}.*/*.exposures.tsv'))):
...
@cli.command()
@click.option('--deconstruction_folder', type=click.Path())
@click.option('--regression_folder', type=click.Path())
@click.option('--sig', type=str)
@click.option('--shuffle', is_flag=True)
def run_ensemble_regression(deconstruction_folder, regression_folder, sig, shuffle):
    ...
    for n_treated in tqdm([10, 25, 50, 100, 150]):
        for replicate in tqdm(range(0, 25)):
            for s in shuffle_iterable:
                try:
                    ensemble_regression(sig, n_treated, replicate, regression_folder,
                                        deconstruction_folder, shuffle=s)
                except:
                    pass
